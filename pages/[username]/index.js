    ...
    <>
      ...
      <div className="container h-full bg-gray-700 flex xl:flex-row flex-col">
        ...
        <div className="grow h-max p-4">
          <div
            className="text-3xl text-gray-300 hover:cursor-pointer hover:text-white my-5 w-max"
            ...
            {data?.map((item, i) => (
              ...
              >
                <div className="flex flex-col items-start ">
                  <h5 className=" text-xl font-medium text-white p-3">
                    {item.name}
                  </h5>
                  <span className="text-sm text-gray-400 ml-5">
                    {item.visibility}
                  </span>
                  <h5 className=" text-md font-medium text-gray-400 p-3">
                    Language: {item.language ? item.language : "unknown"}
                  </h5>
                  <div className="flex align-text-bottom justify-between  p-3">
                    <p className="text-sm font-medium text-gray-400 mr-4">
                      update: {moment(item.pushed_at).format("DD-MM-YYYY")}
                    </p>
                    <p className="text-sm font-medium text-gray-400 bottom-2">
                      Create: {moment(item.created_at).format("DD-MM-YYYY")}
                    </p>
                  </div>
                </div>
              </div>
